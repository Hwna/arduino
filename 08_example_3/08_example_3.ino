...
//  }
  if (dist_raw > dist_min && dist_raw <= 200) {
    aValue = -(2.55) * (dist_raw - 200);
    analogWrite(PIN_LED, aValue);
  } else if (dist_raw < dist_max && dist_raw > 200) {
    aValue = (2.55) * (dist_raw - 200);
    analogWrite(PIN_LED, aValue);
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max) return; // return previous value  when out of range.
  return reading;
